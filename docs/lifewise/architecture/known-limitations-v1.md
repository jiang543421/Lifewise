...
| B-11 | v1.0.3 gap scan (post-v1.0.2) | `/api/ai/chat` SSE 流式聊天端点 plan 有但 v1.0 release cut 时未实现 | MEDIUM (deferred v1.1+) | plan-06-ai §2.6 设计为 SSE streaming chat（前端边收边渲染 Ollama 输出）。v1.0 release cut (`e908c41`) 时 nginx `default.conf` 配置了 `location = /api/ai/chat` 带 `proxy_buffering off` + 24h timeout + `ai_chat` 限流 zone，但 Spring 侧无 `@RequestMapping("/api/ai/chat")` 的 controller——生产 curl 会经 nginx → app → 404。**命名误导警示**：`ChatMessage` / `ChatRole` / `ChatMessageRepository` / `ChatMessageAiAuditLogger` **不是** SSE chat 实现，是审计基础设施（写入 `chat_messages` 表的 SYSTEM 角色记录），被 `AiJobProcessor` + `ConsentVerifier` 通过 `AiAuditLogger` 接口调用，每次 AI 作业/同意决策都留痕。删除这些文件会砍审计链路（plan-06-ai §6 步骤 2.5 + §7.7 隐私约束），禁止按字面 "Chat* = dead code" 误操作。**v1.0.3 处置**：删除 nginx location 块（保留 `ai_chat` zone 给 v1.1+）+ 删除 `ai/dto/ChatRequest.java` + `ai/dto/ChatResponse.java` 两个零引用 DTO；保留审计基础设施 4 文件。**v1.1+ 触发条件**：本地 LLM 升级到支持 streaming 的模型（如 deepseek-r1 流式输出） + 前端 SSE 客户端就绪时按 plan-06-ai §2.6 重新实现，复用 `ChatMessage` 实体做消息留痕。**How to apply**：v1.1+ 落地时先补 SSE controller + `StreamingResponseBody` + Ollama stream API，再用 chat_messages 表 + ChatMessageAiAuditLogger 留痕。**Don't do**：不要在 v1.0.x patch 强行实现 SSE chat（24h 长连接稳定性需额外验证，扩大 blast radius 违反 patch 纪律）。 |

**Backlog note** (after 2026-08-06 v1.3.3 closure): **M1 / M2 / M4 / M5 / M6 / M7 / M8 / B-1 / B-2 / B-3 / B-5 / B-6 / B-7 / B-9 / B-10 are closed** — see §1 closure column (M-series) and §2 (B-series). **B-5/B-6 ledger-only (v1.3)**: 3 release-blockers + 1 IT-side-effect discovery. **B-9/B-10 ledger-only (v1.3.2 false alarm)**: 2 audit-confirmed non-issues from plan module Step 6 audit (consumer idempotency + missing event forwarder). **M6 ledger-only (v1.3.2 verification)**: cross-entity validation 已落地 (`ExpenseService.java:100-103`); access modifier 与项目 7-entity convention 一致 (Budget/Milestone/Task/Plan/DailyReport/Food/Profile 全 public), 收紧需 service 挪 domain 包违反分层 — 决定保留 public。**B-1 ledger-only (v1.3.3)**: nginx path-based X-User-Id header rejection (`map` + 3× `if`), defense-in-depth for v1.1+ multi-user migration. **B-7 ledger-only (v1.3.3)**: AuthController 6 端点 + V50 + EmailService abstraction, spec §5.4 合规. **H3 is decided** via ADR-001 §5 (plan B for v1.0 single-user; plan A mandatory on v1.1 clusterization trigger). **AI module (v1.3.3)**: 6/6 模块全部 promoted to main via PR #18 (`8bc0068` squash `19edc0b`) — OllamaClient + ScopedDataFetcher + AiRateLimiter + 3 controllers + V42/V46/V47 Flyway. **Remaining §2 Open** = **B-8 Web Push (deferred v1.1+ per spec §5 mitigation + CLAUDE.md §1.3 out-of-scope)** + **B-11 SSE chat endpoint (deferred v1.1+ per plan-06-ai §2.6 + v1.0.3 nginx location 移除 commit)**.
